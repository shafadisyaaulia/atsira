...
export default function MyProductsPage() {
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    // Generate ID unik
    const gradeResponse = await fetch(`/api/products/next-batch-id?grade=${formGrade}`);
    const { batchId } = await gradeResponse.json();
    ...
    const { error } = await supabase.from("products").insert([
      {
        ...
        is_raw: isRawType,
        seller_id: user.id,
        qr_batch_id: batchId, // Simpan ID unik
      },
    ]);
            ...
            <form onSubmit={handleAddProduct} className="space-y-6 text-sm">
              ...
              {formType === "finished" && (
                <>
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-extrabold text-stone-900">
                      5. Kualitas / Grade Produk:
                    </label>
                    <select
                      value={formGrade}
                      onChange={(e) => setFormGrade(e.target.value as any)}
                      className="bg-white border-stone-300 text-stone-950 text-sm p-3 focus:ring-2 focus:ring-emerald-600 font-medium rounded-lg h-11 w-full"
                    >
