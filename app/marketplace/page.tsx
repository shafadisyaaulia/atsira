...
const T_MARKET = {
  ...
  searchPlaceholder: { id: "Coba: minyak kadar pa tinggi atau parfum", en: "Try: high PA grade oil or organic perfume" },
  ...
  atsiraBadge: { id: "Atsira Verified", en: "Atsira Verified" },
  arcBadge: { id: "Arc Verified", en: "Arc Verified" },
...
export default function MarketplacePage() {
  ...
  const checkIsArc = (product: any) => {
    return (
      product.badges?.includes("USK Verified") || 
      product.verifiedBy === "arc" || 
      String(product.id).includes("gayowood") || 
      String(product.id).includes("usk")
    );
  };
  ...
  return (
    <PageShell>
                    ...
                    <div>
                      <Link href={`/marketplace/${p.id}`} className="block relative aspect-square overflow-hidden bg-bone-wash">
                        ...
                        <div className="absolute top-3 left-3 flex gap-1.5 flex-wrap">
                          {isArc ? (
                            <Badge variant="usk" className="flex items-center gap-1 shadow-sm text-[10px] bg-sky-50 text-sky-950 border-sky-300">
                              <ShieldCheck className="w-3 h-3 text-sky-600 inline" /> {T_MARKET.arcBadge[currentLang]}
                            </Badge>
                          ) : (
                            <Badge variant="ai" className="flex items-center gap-1 shadow-sm text-[10px] bg-amber-50 text-amber-900 border-amber-200">
                              <Sparkles className="w-3 h-3 text-amber-600 inline" /> {T_MARKET.atsiraBadge[currentLang]}
                            </Badge>
                          )}
                        </div>
                      </Link>
                      <div className="p-4 pb-0">
                        ...
                        <div className="flex items-center gap-1 mb-3">
                          ...
                          <span className="text-xs font-medium text-on-surface-variant">{p.rating || "4.8"} <span className="text-outline">({p.reviewCount || 12})</span></span>
                        </div>
                  ...
                  <Card key={p.id} className="relative overflow-hidden h-full flex flex-col justify-between hover:shadow-xl rounded-2xl border border-surface-container-high transition-all duration-300 bg-white group">
                    <div>
                      <Link href={`/marketplace/${p.id}`} className="block relative aspect-[4/3] overflow-hidden bg-bone-wash">
                        ...
                        <div className="absolute top-3 left-3 flex gap-1.5 flex-wrap">
                          {isArc ? (
                            <Badge variant="usk" className="text-[10px] bg-sky-50 text-sky-950 border-sky-300">
                              <ShieldCheck className="w-2.5 h-2.5 text-sky-600 inline mr-0.5" /> {T_MARKET.arcBadge[currentLang]}
                            </Badge>
                          ) : (
                            <Badge variant="ai" className="text-[10px] bg-amber-50 text-amber-900 border-amber-200">
                              <Sparkles className="w-2.5 h-2.5 text-amber-600 inline mr-0.5" /> {T_MARKET.atsiraBadge[currentLang]}
                            </Badge>
                          )}
                        </div>
                      </Link>
                      <div className="p-4 pb-0">
                        ...
                        <div className="flex items-center justify-between mb-3 bg-stone-50 p-2 rounded-xl border border-stone-100">
                          ...
                          <span className="text-xs font-mono font-bold text-clay-earth">PA {p.coa?.paLevel || 32}%</span>
                        </div>
                        <div className="flex items-baseline justify-between">
                          <p className="font-display font-bold text-primary text-base">
                            {formatCurrency(p.pricePerKg)} <span className="text-xs font-body font-normal text-outline">/kg</span>
                          </p>
                          <span className="text-xs text-outline">{T_MARKET.minOrder[currentLang]} {p.minOrderKg || 5}kg</span>
                        </div>
                      </div>
                    </div>
                    <div className="p-4 pt-3 flex items-center gap-2 border-t border-stone-50 mt-4 relative z-20">
      ...
      <CartNotificationModal isOpen={modalOpen} onClose={() => setModalOpen(false)} productName={addedProductName} />
    </PageShell>
  );
}